...
export class ProductRepository {
  async create(data: {
    ...
    try {
      ...
      const pRes = await client.query(
        `INSERT INTO products (category_id, title, description, highlights)
         VALUES ($1, $2, $3, $4::text[])
         RETURNING id`,
        [data.categoryId, data.title, data.description ?? null, data.highlights],
      );
      const productId = pRes.rows[0].id as string;
  ...
  async getById(id: string) {
    const res = await pool.query(
      `SELECT
       ...
       WHERE p.id = $1
       ORDER BY ad.name ASC NULLS LAST`,
      [id],
    );
    return this.mapProductRows(res.rows);
  }

  async search(whereSql: string, params: any[], take: number, skip: number) {
  ...
  async findIdsBySearch(whereSql: string, params: any[]) {
    const res = await pool.query(
      `SELECT p.id FROM products p WHERE ${whereSql} LIMIT 5000`,
      params,
    );
    return res.rows.map((r) => r.id as string);
  }

  async getValuesForProducts(productIds: string[]) {
